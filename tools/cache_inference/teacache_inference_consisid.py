...
def main(args):
    ...
    pipe = ConsisIDPipeline.from_pretrained(ckpts_path, torch_dtype=torch.float16)
    pipe.to("cuda")

    print("="*60)
    print("模型配置信息:")
    print(f"Transformer config: {pipe.transformer.config}")
    print(f"Patch size: {getattr(pipe.transformer.config, 'patch_size', 'Not found')}")
    print(f"Sample height: {getattr(pipe.transformer.config, 'sample_height', 'Not found')}")
    print(f"Sample width: {getattr(pipe.transformer.config, 'sample_width', 'Not found')}")
    print(f"In channels: {getattr(pipe.transformer.config, 'in_channels', 'Not found')}")
    vae_scale_factor = pipe.vae_scale_factor_spatial
    print(f"VAE Scale Factor (spatial): {vae_scale_factor}")
    print("="*60)

    #我添加的
    # 2. 仅启用transformer梯度检查点（不启用xFormers和CPU卸载）
    if hasattr(pipe.transformer, "enable_gradient_checkpointing"):
        pipe.transformer.enable_gradient_checkpointing()
        print("transformer梯度检查点启用完成")  # 加打印，确认到这一步没崩溃
    else:
        for sub_module in pipe.transformer.modules():
            if hasattr(sub_module, "enable_gradient_checkpointing"):
                sub_module.enable_gradient_checkpointing()
        print("transformer子模块梯度检查点启用完成")

    # 3. 加一个“测试打印”，看是否能走到这一步
    print("所有基础优化启用完成，准备生成...")

    print("="*60)
    print("验证修改是否生效")
    print(f"ConsisIDPipeline类文件位置: {ConsisIDPipeline.__module__}")
    print(f"decode_latents方法: {hasattr(pipe, 'decode_latents')}")
    
    # 检查方法是否是我们修改的版本
    import inspect
    source = inspect.getsource(pipe.decode_latents)
    if "优化版的VAE解码方法" in source:
        print("decode_latents方法已修改")
    else:
        print("decode_latents方法未修改")
    print("="*60)
    #结束添加
    ...
    #===========清理人脸部分的模型===========
    print("人脸特征提取完成，立即清理人脸识别模型...")
    
    # 立即清理人脸识别模型
    models_to_cleanup = [
        face_helper_1, face_helper_2, face_clip_model, face_main_model, 
    ...
    for i, model in enumerate(models_to_cleanup):
        if model is not None:
            del model
            print(f"已清理人脸模型 {i+1}/{len(models_to_cleanup)}")
    ...
    video_path = f"{output_path}/{file_count:04d}.mp4"
