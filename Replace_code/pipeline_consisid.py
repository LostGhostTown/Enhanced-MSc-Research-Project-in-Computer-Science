...
class ConsisIDPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    ...
    def prepare_latents(
        ...
        print("================================")
        print(f"vae_scale_factor_temporal: {self.vae_scale_factor_temporal}")
        print("================================")
        shape = (
            batch_size,
            num_frames,
        ...
        # ==================== Modification 1: Add VRAM cleanup in prepare_latents ====================
        print("="*60)
        print("[prepare_latents] Starting to clean up original input tensors")
        pre_mem = torch.cuda.memory_allocated() / 1024**2 if device.type == "cuda" else 0
        
        # Clean up original input tensors that are no longer needed
        if isinstance(image, torch.Tensor):
            del image
        if kps_cond is not None and isinstance(kps_cond, torch.Tensor):
            del kps_cond
            
        if device.type == "cuda":
            torch.cuda.empty_cache()
            
        post_mem = torch.cuda.memory_allocated() / 1024**2 if device.type == "cuda" else 0
        print("Move VAE to CPU")
        if hasattr(self, 'vae') and self.vae is not None:
        # Save the current device info for later restoration
            self._vae_original_device = self.vae.device
            self.vae.to('cpu')
            print("VAE has been moved to CPU")
        if device.type == "cuda":
            print(f"[prepare_latents] GPU VRAM before cleanup: {pre_mem:.2f}MB")
            print(f"[prepare_latents] GPU VRAM after cleanup: {post_mem:.2f}MB")
            print(f"[prepare_latents] Freed VRAM: {pre_mem - post_mem:.2f}MB")
        print("[prepare_latents] Cleanup completed")
        print("="*60)
        print("="*60)
        print("VRAM details after prepare_latents cleanup:")
        print(torch.cuda.memory_summary(device=device, abbreviated=True))  # Print concise summary
        print("="*60)
    ...
    def decode_latents(self, latents: torch.Tensor) -> torch.Tensor:
        """Optimized VAE decoding method with VRAM management"""
        print("="*60)
        print("[decode_latents] Starting VAE decoding phase")
        ...
        try:
            ...
            # Ensure cleanup is executed regardless
            import gc
            del latents
            gc.collect()
            ...
            if device.type == "cuda":
                ...
                print(f"[decode_latents] GPU VRAM after decoding: {post_mem:.2f}MB")
                print(f"[decode_latents] Peak VRAM: {torch.cuda.max_memory_allocated()/1024**2:.2f}MB")
            print("[decode_latents] VAE decoding completed")
            print("="*60)
    ...
    def __call__(
        ...
        # 1.text_encoder
        pre_clean_mem = torch.cuda.memory_allocated() / 1024**2
        if hasattr(self, 'text_encoder') and self.text_encoder is not None:
            del self.text_encoder
            self.text_encoder = None
            print("The text_encoder model has been completely cleaned up.")
        # 3.tokenizer
        if hasattr(self, 'tokenizer') and self.tokenizer is not None:
            del self.tokenizer
            self.tokenizer = None
            print("The tokenizer has been completely cleaned up.")
        import gc
        gc.collect()
        if device.type == "cuda":
            torch.cuda.empty_cache()
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                latents = latents.to(prompt_embeds.dtype)
                 # ==================== input in timestep ====================
                intermediate_vars_to_clean = []

                
                if 'latent_image_input' in locals() and latent_image_input is not None:
                    intermediate_vars_to_clean.append(latent_image_input)
                if 'timestep' in locals() and timestep is not None:
                    intermediate_vars_to_clean.append(timestep)

               
                if do_classifier_free_guidance and 'noise_pred_uncond' in locals() and noise_pred_uncond is not None:
                    intermediate_vars_to_clean.append(noise_pred_uncond)
                if do_classifier_free_guidance and 'noise_pred_text' in locals() and noise_pred_text is not None:
                    intermediate_vars_to_clean.append(noise_pred_text)

                
                if 'noise_pred' in locals() and noise_pred is not None:
                    intermediate_vars_to_clean.append(noise_pred)

                # 4. collect
                for tensor in intermediate_vars_to_clean:
                    del tensor
                gc.collect()

                if i < len(timesteps) - 1:  
                    if 'latent_model_input' in locals() and latent_model_input is not None:
        ...
        # ====================transformer====================
        print("="*60)
        print("Denoising completed, starting to clean up the text_encoder and transformer models...")
        ...
        if hasattr(self, 'transformer') and self.transformer is not None:
            ...
            print("transformer model has been completely cleaned up")
        ...
        if device.type == "cuda":
            ...
            post_clean_mem = torch.cuda.memory_allocated() / 1024**2
            print(f"   Before cleaning: {pre_clean_mem:.2f}MB")
            print(f"   After cleaning: {post_clean_mem:.2f}MB") 
            print(f"   Release: {pre_clean_mem - post_clean_mem:.2f}MB")
        
        print("Model cleanup completed, preparing for VAE decoding")
        print("="*60)
        #===================== ====================
        # ==================== 4：before VAE====================
        print("="*60)
        print("Preparing to enter the VAE decoding stage, starting to thoroughly clean up the video memory...")
        pre_clean_mem = torch.cuda.memory_allocated() / 1024**2 if device.type == "cuda" else 0

        # Create a list of variables to clean up
        cleanup_list = []

        # Add variables to the cleanup list
        if 'prompt_embeds' in locals() and prompt_embeds is not None:
            cleanup_list.append(prompt_embeds)
        if 'negative_prompt_embeds' in locals() and negative_prompt_embeds is not None:
            cleanup_list.append(negative_prompt_embeds)
        if 'image_latents' in locals() and image_latents is not None:
            cleanup_list.append(image_latents)
        if 'image_rotary_emb' in locals() and image_rotary_emb is not None:
            cleanup_list.append(image_rotary_emb)

        for var in cleanup_list:
            del var
        ...
        post_clean_mem = torch.cuda.memory_allocated() / 1024**2 if device.type == "cuda" else 0
        if device.type == "cuda":
            print(f"Before cleaning: {pre_clean_mem:.2f}MB")
            print(f"After cleaning: {post_clean_mem:.2f}MB")
            print(f"Release: {pre_clean_mem - post_clean_mem:.2f}MB")
        print("Denoising stage variable cleanup completed, entering VAE decoding stage")
        print("="*60)
        print("="*60)
        print("Details of video memory before VAE decoding:")
        print(torch.cuda.memory_summary(device=device, abbreviated=True))
        print("="*60)
        # ====================4====================

        if not output_type == "latent":
            print("[decode_latents] Moving VAE back to GPU...")
            if hasattr(self, 'vae') and self.vae is not None:
                target_device = latents.device
                self.vae.to(target_device)
                print(f"VAE moved to {target_device}")
