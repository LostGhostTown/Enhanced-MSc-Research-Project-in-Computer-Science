...
class ConsisIDPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    ...
    def prepare_latents(
        ...
        latents = latents * self.scheduler.init_noise_sigma
         ...
         # ==================== 修改点1：在prepare_latents中添加显存清理 ====================
        print("="*60)
        print("[prepare_latents] 开始清理原始输入张量")
        ...
        # 保存当前设备信息，以便后续恢复
            ...
            print("VAE已移动到CPU")
        if device.type == "cuda":
            print(f"[prepare_latents] 清理前GPU显存：{pre_mem:.2f}MB")
            print(f"[prepare_latents] 清理后GPU显存：{post_mem:.2f}MB")
            print(f"[prepare_latents] 释放显存：{pre_mem - post_mem:.2f}MB")
        print("[prepare_latents] 清理完成")
        print("="*60)
        print("="*60)
        print("prepare_latents清理后显存详情：")
        print(torch.cuda.memory_summary(device=device, abbreviated=True))  # 打印简洁版摘要
        print("="*60)
    ...
    def decode_latents(self, latents: torch.Tensor) -> torch.Tensor:
        """优化版的VAE解码方法，添加显存管理"""
        print("="*60)
        print("[decode_latents]  开始VAE解码阶段")
        ...
        try:
            ...
            if device.type == "cuda":
                ...
                print(f"[decode_latents] 解码后GPU显存：{post_mem:.2f}MB")
                print(f"[decode_latents] 峰值显存：{torch.cuda.max_memory_allocated()/1024**2:.2f}MB")
            print("[decode_latents]  VAE解码完成")
            print("="*60)
